...
function render() {
  ...
  if (searchText === "") {
    searchResults.textContent = "All episodes";
    makePageForEpisodes(state.episodes);
    return;
  }

  const filteredEpisodes = state.episodes.filter((episode) => {
    return episode.name.toLowerCase().includes(searchText) || episode.summary.toLowerCase().includes(searchText);
  });

  searchResults.textContent = `${filteredEpisodes.length} episodes`;
